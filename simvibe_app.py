...
def mutate_smiles(smiles: str, n_mut=3):
    ...
    try:
        ...
        logger.debug(f"[mutate_smiles] {smiles} -> {new_smi}")
        return [new_smi]
    except Exception as e:
        logger.debug(f"[mutate_smiles] Sanitization failed for mutated {smiles}: {e}")
        return []
    ...
    if run:
        ...
        if mode == "Greedy (Top-K)":
            ...
            for cycle in range(n_cycles):
                ...
                detail_box.markdown(
                    f"Best score this cycle: `{best_score:.3f}`  "
                ...
                logger.info(
                    ...
                    f"best_score={best_score:.3f} | best_smi={best_smi}"
                )

                # Keep top 60
                candidates = {
                    s for s, _ in sorted(scores, key=lambda x: -x[1])[:60]
                }
            ...
            for step in range(n_steps):
                # Propose mutation from current molecule
                proposals = mutate_smiles(
                    current_smi, n_mut=random.randint(1, 4)
                )
                if not proposals:
                    continue
                ...
                if step == 0 or step == n_steps - 1 or (step + 1) % 10 == 0:
                    logger.info(
                        ...
                        f"current_score={current_score:.3f} | delta={last_delta:.3f} | "
